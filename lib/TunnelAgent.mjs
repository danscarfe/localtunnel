...
class TunnelAgent extends Agent {
  ...
  listen() {
    ...
    server.on("drop", this._onDrop.bind(this));
    server.on("error", (err) => {
  ...
  _onDrop() {
    this.closed = true;
    this.debug("droped tcp socket");
    // flush any waiting connections
    for (const conn of this.waitingCreateConn) {
      conn(new Error("droped"), null);
    }
    this.waitingCreateConn = [];
    this.emit("end");
  }
  ...
  _onConnection(socket) {
    console.log("Socket connected", this.connectedSockets, this.maxTcpSockets);
