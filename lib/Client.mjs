...
class Client extends EventEmitter {
  constructor(options) {
    ...
    const grace_timeout = (this.grace_timeout = options.grace_timeout || 10000);
    ...
    this.graceTimeout = setTimeout(() => {
      this.debug("grace timeout exceed for connection %s", grace_timeout);
      this.close();
    }, grace_timeout).unref();
    ...
    agent.on("offline", () => {
      ...
      this.graceTimeout = setTimeout(() => {
        this.debug("grace timeout exceed after offline %s", grace_timeout);
        this.close();
      }, grace_timeout).unref();
    });
